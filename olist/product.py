...
class Product:
    ...
    def get_product_cat(self, agg="mean"):
        ...
        # Keep only numeric columns to aggregate (drop 'product_id', 'category')
        columns = list(products.select_dtypes(exclude=['object']).columns)

        # Apply the chosen aggregation (mean/median) to every numeric column
        agg_params = dict(zip(columns, [agg] * len(columns)))

        # quantity should always be summed (total units sold per category)
